...
| 4 | 2026-08-14 | **Noticed when the site is down.** GitHub Pages was switched off on **2026-08-12** — last successful deployment 14:27 UTC on `fe34326`, and none since. The scraper never missed a beat: it ran on 08-13 and 08-14, committed real data (~1,300 lines of churn each day), and reported success both times, while `cooknrice.github.io/precon-tracker` returned **404 to every visitor for ~2 days**. Every workflow was green the whole time, because nothing in CI had ever looked at the *published site* — only at the data files sitting in the repo. Everything here tested the data; nothing tested the delivery. Added `site-health.yml`: a daily 18:00 UTC check (well clear of the scrape's late-starting 13:00 cron) that the Pages URL returns 200, and that the **served** `prices.json` parses, reports a non-zero `deck_count`, and is no more than 26h old — so a silent Pages outage *and* a silently missed scrape both trip it. On failure it opens a de-duplicated issue naming the likely cause and the one-line re-enable command. Pages itself was re-enabled the same day (`source: main /`). | YAML parsed and step list asserted; freshness logic dry-run against the live site before shipping (`updated_at` 2026-08-14T14:22Z, age 8.4h, 115 decks, 4 vendors → PASS); restoration confirmed end-to-end — HTTP 200, 134,567-byte `index.html`, served `prices.json` current; issue-body quoting checked so the embedded backticks and `<<<` heredoc cannot trigger command substitution |
